...
function submit() {
    ...
    $("#payment").append('<form action="" method=""> <script src="https://checkout.stripe.com/checkout.js" class="stripe-button" data-key="pk_z0cCzBIDAkyhm2vUGcOSDevqqN1Bq" data-amount=' + budget * 100 + '</script> </form>')
...
function generateRestaurantHtml(title, description, image, priceLow, priceHigh, id){
    ...
    return '<div class="col-sm-4 col-lg-2 resta-card" id="' + id + '"><div class="thumbnail"> <img src="' + image + '">  <p class="title-header">' + title +'</p> <p class="price-header">$' + priceLow + '-$' + priceHigh + '</p></div><div class="resta-description"><p>' + description + '</p> </div></div>';
}
